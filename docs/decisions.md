...
### TreasureChest 구조 — AC_BaseItem 상속 vs 독립 AActor
- **선택:** `AC_TreasureChest : public AC_BaseItem`, itemID/DataTable 미사용(`AC_MoneyItem`과 동일 선례), `OnInteract`만 override
- **대안:** 독립 `AActor`로 상자 전용 입력/UI 파이프라인 신규 구현
- **선택 이유:** `AC_PlayerController::overlappingItems`가 `TArray<TWeakObjectPtr<AC_BaseItem>>`로 하드타입되어 있어, 상속하지 않으면 플레이어 파트(PlayerController)를 인터페이스 기반으로 리팩토링해야 함 — 상자 기능 범위를 넘는 팀 경계 변경
- **트레이드오프:** 상자는 엄밀히 "아이템"이 아닌데 `AC_BaseItem` 계층에 속하게 됨(itemID/InitItem 경로는 사용 안 함)

### TreasureChest 스폰 주체 — 상자 자신 vs UC_InventoryComponent
- **선택:** `AC_TreasureChest::OnInteract`가 직접 loot 계산 + `SpawnActor` 수행
- **대안:** 인벤토리 컴포넌트에 위임
- **선택 이유:** 인벤토리 직접 지급이 아니라 월드에 픽업으로 흩뿌리는 것이 요구사항이라 인벤토리 컴포넌트가 관여할 이유가 없음. "UseItem 구현 위치" 결정과 동일한 원칙(불필요한 간접화 회피)
- **트레이드오프:** 없음

### TreasureChest Value 분배 — 배정 순서와 기준
- **선택:** ① 장비 필수 상자는 장비 풀 중 value가 가장 낮은 것(동률 시 랜덤)을 최우선으로 선차감 ② 돈 몫은 `chestValue` 전체 기준 1~50% 랜덤 ③ 장비+돈이 예산을 넘으면 돈 몫을 줄여서 맞춤(장비 선차감 자체가 예산보다 큰 경우만 예외) ④ 남은 value로 아이템 반복 추첨, 자투리는 돈에 합산
- **대안:** 돈 몫을 먼저 배정한 뒤 남은 value에서 장비를 추첨하거나, 장비를 완전 랜덤으로 선차감
- **선택 이유:** 돈을 먼저 배정하면 장비 필수 상자의 "최소 1개 보장"이 깨질 수 있음(예: chestValue=70, 최저 장비=50인데 돈에 21 이상 배정되면 장비를 못 뽑는 상황 발생). 장비 선차감을 완전 랜덤으로 하면 비싼 장비가 뽑혀 예산이 크게 초과될 리스크가 커짐 — "가장 싼 장비"로 고정해 리스크 최소화
- **트레이드오프:** 장비 선차감 자체가 예산을 넘는 극단적인 `chestValue` 설정에서는 예산 초과가 허용됨(레벨 디자이너가 `chestValue`를 장비 최소값 이상으로 설정하도록 가이드 필요)

### 런타임 스폰 픽업의 오버랩 인식 — 델리게이트 단독 vs 명시적 재확인
- **선택:** `AC_BaseItem::RefreshOverlapState()` 신설, 스폰 주체가 데이터 초기화 직후 명시적으로 호출
- **대안:** `OnComponentBeginOverlap` 델리게이트만 사용(수정 없이 방치), 혹은 스폰을 한 프레임 뒤로 지연(Timer/다음 틱)해 델리게이트가 정상 작동하길 기대
- **선택 이유:** 델리게이트가 `BeginPlay()`에서 바인딩되는데 런타임 스폰 시 초기 오버랩 판정이 그보다 먼저 처리되어 이벤트를 놓치는 경우가 실사용에서 확인됨(TreasureChest 드랍 UI 미표시/0값 표시 버그). 한 프레임 지연 방식은 딜레이가 체감되고 원인 해결이 아니라 우회일 뿐이라 배제
- **트레이드오프:** `AC_BaseItem`(다른 모든 픽업이 상속하는 공용 베이스)을 수정 — TreasureChest 범위를 넘는 변경이지만, 문제 자체가 "런타임 SpawnActor로 생성되는 모든 픽업"에 공통이라 베이스에 두는 것이 맞음

### 지면 스폰 실패 폴백 — 미보정 좌표 vs 재탐색 체인 vs 스폰 취소
- **선택:** 1차 트레이스 실패 시 상자 자신의 위치 바로 아래로 2차 재탐색, 그마저 실패해야 미보정 좌표 사용
- **대안:** TrapZone처럼 실패 시 스폰 취소, 또는 그냥 미보정 좌표(Candidate) 사용
- **선택 이유:** 흩뿌리기는 개별 지점 실패가 있어도 전체 루팅 결과(아이템 개수)는 보장돼야 하므로 스폰 취소는 부적합. 미보정 좌표를 쓰면 상자 높이에 뜬 채로 스폰되는 버그가 됨(실사용에서 확인). 상자 자신은 반드시 바닥 위에 있으므로 상자 위치 재탐색은 사실상 항상 성공
- **트레이드오프:** 트레이스 호출이 최대 2배로 늘어남(체감 비용 미미)
