...
46. **Paragon 에셋이 카메라에서 멀어지면 캐릭터가 뭉개져 보일 때** — LOD 전환이 트리거지만 진짜 원인은 **LOD의 `LODMaterialMap` 손상**인 경우가 있음. 섹션끼리 머티리얼이 뒤바뀌어 형태는 맞는데 재질만 어긋남(예: Terra — 깃털 장식이 은색 금속, 도끼 날이 갈색 가죽, 투구가 살색). 폴리곤 감소로 인한 실루엣 저하와 헷갈리기 쉬움.
