...
- 2026-06-26: 인벤토리 파일 위치 재정리(사용자 요청) — C++ `Source/ProjectBBK/Monster/UI/Inventory/` → `Source/ProjectBBK/Inventory/`(GAS·Items·LevelSystem과 동일 최상위), Content `/Game/Monster/UI/Inventory/` → `/Game/Inventory/`. 대상: C_InventoryComponent·C_InventoryWidget·C_InventorySlotWidget·C_InventoryTester(.h/.cpp) / WBP_Inventory·WBP_InventorySlot·T_InventoryBG·T_InventorySlot·T_MoneyPouch. include 경로 수정(인벤토리 내부 `../../../Items/ItemData.h`→`../Items/ItemData.h`, C_PlayerController.cpp·C_BaseItem.cpp 외부 참조). 옛 폴더(Source/Content 양쪽) 삭제 완료. VS 프로젝트 파일 재생성+빌드 필요.
- 2026-06-26: 디버그용 `AC_InventoryTester`(C_InventoryTester.h/.cpp) 삭제(사용자 요청) — 키 할당 전 레벨에 직접 띄워 검증하던 용도, 이제 G키 상호작용·I키 인벤토리 토글이 연결되어 불필요. 코드 참조·BP 서브클래스·배치 인스턴스(Untitled·MonsterTest 양쪽) 모두 없음 확인 후 제거. VS 프로젝트 파일 재생성+빌드 필요.
